...
> 文档用途：在全新会话中快速恢复项目上下文，避免重复排查与重复设计。  
> 交接时间：2026-07-23  
> 项目路径：`E:\agent_aurum`  
> 当前阶段：阶段一“架构和安全底座”已完成，阶段二尚未开始开发。
...
当前没有完成浏览器前端、财务业务接口、知识库管理、文档解析与入库、LangGraph 工作流、
RAG 检索问答、引用展示和历史会话接口。这些目录和数据表虽然已经预留，但不能视为功能已实现。
...
- 初始管理员密码 `123456` 仅适用于开发环境，生产配置会拒绝该默认值。
...
- 生产环境若仍使用示例 JWT 密钥、`123456` 管理员密码或开启 Debug，配置校验会拒绝启动；
