...
function resetForm(): void {
  ...
  Object.assign(form, {
    ...
    transaction_type: 'expense' as TransactionType,
    amount: 0,
    currency: firstAccount?.currency ?? 'CNY',
    category: '',
    description: '',
    transaction_date: dayjs(),
  })
...
async function importFile(): Promise<void> {
  ...
  try {
    ...
    if (result.errors.length) {
      ...
      message.success(`导入完成：新增 ${result.imported_rows} 行，跳过 ${result.skipped_rows} 行`)
    }
...
<template>
  <div class="page-shell">
    <div class="page-heading">
      <div>
        <h1>收支明细</h1>
        <p>记录、检索和导入现金流。每次新增、修正或删除都会原子更新对应账户余额。</p>
      </div>
      <a-space>
        <a-button size="large" @click="importOpen = true"><FileExcelOutlined />导入表格</a-button>
        <a-button type="primary" size="large" @click="openCreate"><PlusOutlined />记录交易</a-button>
      </a-space>
    </div>

    <a-card class="surface-card filter-card" :bordered="false">
      <div class="filter-grid">
        <a-input v-model:value="filters.search" allow-clear placeholder="搜索分类或备注" @press-enter="search">
          <template #prefix><SearchOutlined /></template>
        </a-input>
        <a-select v-model:value="filters.account_id" allow-clear placeholder="全部账户">
          <a-select-option v-for="account in accounts" :key="account.id" :value="account.id">
            {{ account.name }}
          </a-select-option>
        </a-select>
        <a-select v-model:value="filters.transaction_type" allow-clear placeholder="收支类型">
        ...
        <a-button type="primary" @click="search"><FilterOutlined />筛选</a-button>
    ...
    <a-card class="surface-card table-card" :bordered="false">
      <a-table
        ...
        :scroll="{ x: 930 }"
        row-key="id"
        @change="(pagination: { current?: number; pageSize?: number }) => changePage(pagination.current ?? 1, pagination.pageSize ?? 20)"
      >
        <template #bodyCell="{ column, record }: { column: { key: string }; record: Transaction }">
          <template v-if="column.key === 'date'">
            <span class="date-cell">{{ formatDate(record.transaction_date, 'MM-DD') }}</span>
          ...
          <template v-else-if="column.key === 'type'">
            <a-tag :color="record.transaction_type === 'income' ? 'green' : 'red'">
              {{ record.transaction_type === 'income' ? '收入' : '支出' }}
            </a-tag>
          </template>
          <template v-else-if="column.key === 'amount'">
            <strong
              :class="record.transaction_type === 'income' ? 'money-positive' : 'money-negative'"
            >
          ...
          <template v-else-if="column.key === 'source'">
            <span class="muted">{{ record.source === 'import' ? '表格导入' : '手工记录' }}</span>
          </template>
          <template v-else-if="column.key === 'actions'">
            <a-space>
              ...
              <a-button type="link" danger class="table-action" @click="deleteTransaction(record)">
                <DeleteOutlined />
              </a-button>
            </a-space>
          </template>
    ...
    >
      <a-form layout="vertical" :model="form">
        ...
        <a-form-item label="账户" required>
          ...
          >
            <a-select-option v-for="account in activeAccounts" :key="account.id" :value="account.id">
        ...
        <a-form-item label="分类" required>
          <a-input v-model:value="form.category" placeholder="例如：餐饮、工资、交通" />
        </a-form-item>
      ...
      <div class="import-panel">
        ...
        <a-form layout="vertical">
          <a-form-item label="目标账户" required>
            <a-select v-model:value="importAccountId" placeholder="选择导入账户">
              <a-select-option
          ...
          <a-form-item label="导入文件" required>
            <label class="file-picker">
              ...
              <input type="file" accept=".csv,.xlsx" @change="selectImportFile" />
            </label>
          </a-form-item>
...
.filter-grid {
  display: grid;
  grid-template-columns: minmax(190px, 1.5fr) minmax(140px, 1fr) 120px minmax(230px, 1.3fr) auto auto;
  gap: 10px;
}
