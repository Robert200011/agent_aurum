...
> 编写日期：2026-07-23
> 最后更新：2026-08-03（阶段六工程完成并通过 PR #11 合入 `master`）
...
| 阶段五：个人财务 Agent | 已完成 | 2026-08-02 | 只读工具、财务证据、数值 Grounding、评测集和真实浏览器交付验收 |
| 阶段六 | 工程完成并已合入 `master` | 2026-08-03 | 企业级加固、生产部署资产、蓝绿发布与回滚演练已落地；正式公网发布仍待候选环境验收 |
| 阶段七 | 尚未开始 | — | 后续增强 |

阶段一稳定基线为提交 `933390a`，阶段二通过合并提交 `3bfb9b0` 进入 `master`，阶段四
基线为 `7b99307`，阶段五完成基线为 `571192d`。阶段六通过 GitHub PR #11 squash 合并，
当前功能基线为 `e8aa6ea`。本地 `master` 已与 `origin/master` 对齐。

截至 2026-08-03，阶段一至阶段六的工程开发已经完成。系统已形成用户与租户安全底座、
个人财务账本、六类文档入库、Hybrid Retrieval、可信引用 RAG、多会话 SSE、只读个人
财务 Agent，以及配额、脱敏日志、Trace、指标告警、安全缓存、回归门禁、备份恢复和
蓝绿发布/回滚闭环。阶段六交付的是可部署、可观测、可恢复、可回退的单机生产基线；
真实 Provider、目标服务器容量、正式域名/TLS、外部密钥、异地备份和人工切流仍属于
候选环境上线门禁，不应表述为已经完成公网生产发布。
...
- [x] 时间解析边界：月末、年末、闰年、时区和相对时间；
- [x] 财务工具 Schema、参数上限、空结果和异常结果；
- [x] Decimal 精度、预算执行率、持仓盈亏和汇率换算；
- [x] 纯知识、纯财务、投资、混合和高风险问题路由；
- [x] 开支异常与预算建议在正常、零基数和样本不足情况下的行为；
- [x] 用户 A 无法通过问题、工具参数、Checkpoint 或恢复接口查询用户 B 数据；
- [x] 模型伪造 `user_id`、工具名、金额、行情和引用时被拒绝；
- [x] SSE 状态、停止、恢复、重试、重新生成和历史消息展示；
- [x] 财务证据、知识引用、风险提示和数据更新时间持久化；
- [x] 真实浏览器端的最小财务 Agent 和混合回答冒烟。
...
P5.6 于 2026-08-02 完成：回答图升级为 `finance-agent-p5.6-v1`，新增服务端财务数值与
工具名 Grounding，模型伪造身份、写工具、金额、行情和引用时不能进入最终消息；首次输出
不合格时最多使用同一受控证据重答一次，第二次仍不合格即安全失败。可问答项目现在至少
包含一个已发布、启用且具备 Embedding 的 Chunk，避免创建无法检索知识的会话。版本化
评测集 `phase5-finance-agent.json` 达到 16/16；真实 Edge 浏览器通过纯财务及财务/知识
混合回答，混合轮持久化 1 次只读工具调用、1 条财务证据和 3 条可信引用。阶段五 API
契约、验收报告和当前交接文档均已更新。168 项后端测试（含真实 PostgreSQL）、18 项
前端测试、Ruff、Mypy、Alembic、前端生产构建、Playwright 和 npm audit 均已通过，
阶段五完成。

### 阶段六：企业级加固

阶段六采用 P6.1～P6.5 五个可独立验收批次，先完成单机生产基线，不在本阶段引入
Kubernetes、多地域容灾、复杂计费或新的 Agent 写能力。详细范围、顺序和完成标准见
[阶段六企业级加固开发方案](./aurum-agent-phase-6-plan.md)。

| 批次 | 主要范围 |
| --- | --- |
| P6.1 | 日志脱敏、OpenTelemetry、Prometheus、Grafana 和基础告警 |
| P6.2 | 用户/模型配额、并发控制和最小安全缓存 |
| P6.3 | RAG 回归、Prompt Injection、安全与压力测试门禁 |
| P6.4 | PostgreSQL/对象存储备份、恢复、校验和演练 |
| P6.5 | 生产部署资产、灰度发布、回滚和最终验收 |
...
阶段六 P6.1～P6.5 工程开发和本地演练已经完成。正式公网发布仍须在目标服务器补做真实
Provider 冒烟、目标容量负载、外部密钥/异地备份配置和人工切流批准。
最终代码已通过 PR #11 squash 合入 `master`，阶段六功能基线为 `e8aa6ea`。
