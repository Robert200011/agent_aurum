...
| 阶段三：知识库管理 | 进行中 | — | 第 1～3 步及审计整改完成；第 4 步完成 Markdown/TXT 最小入库闭环 |
| 阶段四至阶段七 | 尚未开始 | — | RAG、财务 Agent、完整企业级加固和后续增强 |

阶段一稳定基线为提交 `933390a`，阶段二通过合并提交 `3bfb9b0` 进入 `master`。
截至 2026-07-28，当前可交付范围是“阶段一 + 阶段二 + 对应 Web 前端 + 阶段三
知识库管理后端第 1～3 步 + Markdown/TXT 到 pgvector 的首个入库闭环”。PDF、DOCX、
CSV、XLSX 入库、知识库管理前端、LangGraph、RAG 问答和 Agent 工具编排尚未完成。
...
- [ ] CSV、XLSX 解析；
- [x] 异步任务；
- [x] Markdown/TXT 确定性分块；
- [x] DashScope Embedding 适配器与批处理流水线（真实 Key 冒烟待补）；
...
阶段六尚未整体完成。上方已勾选的是阶段一、二期间提前落地的安全基线，不能替代后续
